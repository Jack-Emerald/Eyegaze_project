...
class LatentGestureModel(PyroModule):
    def __init__(self, base_model, num_latents, num_classes):
        super().__init__()
        self.base_model = base_model  # Pass in a GestureCNN instance
        self.num_latents = num_latents
        self.num_classes = num_classes
        self.feature_dim = 100  # output of base_model.fc1
        self.classifier = PyroModule[nn.Linear](self.feature_dim + num_latents, num_classes)

    def forward(self, x, y=None):
        batch_size = x.size(0)
        z_probs = torch.ones(batch_size, self.num_latents, device=x.device) / self.num_latents

        with pyro.plate("data", batch_size):
            z = pyro.sample("z", dist.Categorical(z_probs))
            z_one_hot = F.one_hot(z, num_classes=self.num_latents).float().to(x.device)

            # Extract CNN features BEFORE output layer
            x_feat = self.base_model.pool1(F.relu(self.base_model.conv2(F.relu(self.base_model.conv1(x)))))
            x_feat = self.base_model.pool2(F.relu(self.base_model.conv3(x_feat)))
            x_feat = self.base_model.pool3(F.relu(self.base_model.conv4(x_feat)))
            x_feat = x_feat.view(x_feat.size(0), -1)
            x_feat = F.relu(self.base_model.fc1(x_feat))
            x_feat = self.base_model.dropout(x_feat)

            # Concatenate CNN features with latent variable
            combined = torch.cat([x_feat, z_one_hot], dim=1)

            logits = self.classifier(combined.to(self.classifier.weight.device))
...
def guide(x, y=None, num_latents=3):
    ...
    alpha_q = alpha_q.to(x.device)  # ensure same device
    with pyro.plate("data", x.size(0)):  # match model
...
def train_with_latent_variable(trainX, trainy, testX_list, testy, device, batch_size=48, num_latents=3, epochs=30):
    set_seed()
    ...
    cnn_model = GestureCNN(n_timesteps, n_features, n_outputs).to(device)
    model = LatentGestureModel(cnn_model, num_latents=3, num_classes=6).to(device)

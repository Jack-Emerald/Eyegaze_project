...
class GestureDataProcessor:
    ...
    def generate_graph(self, file_path):
        ...
        step = 7*int(self.stepLen)

        # Split the combined_list into sublists of size 448 (? * 64)
        for i in range(0, len(combined_list), step):
            time_step_data = combined_list[i:i + step]
            if len(time_step_data) == step:
                sublist = [time_step_data[i:i + 7] for i in range(0, len(time_step_data), 7)]
    ...
    def build_attention_model(self, n_timesteps, n_features, n_outputs):
        ...
        # Apply GlobalAveragePooling to reduce dimensions
        attention_out = layers.GlobalAveragePooling1D()(attention)

        # Apply dense layer to attention output
        attention_out = layers.Dense(100, activation = 'relu')(attention_out)
    ...
    def evaluate_model(self, trainX, trainy, testX, testy):
        ...
        model = self.build_attention_model(n_timesteps, n_features, n_outputs)
